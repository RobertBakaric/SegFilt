 ...
 */
  ...
  * source code. Therefore  labeles are marked using same or similar variable names. 
  * 
  * */
...
template <typename Tint>
SEG<Tint>::SEG(){
  unordered_map<string,Tint> para;
  SetParamaters(para);
  alpha= MakeAlpha();
}; 
...
template <typename Tint>
string SEG<Tint>::SegFilt(string str){
...
void SEG<Tint>::SetParamaters(Targ &arg){
  ...
  (arg.find("window") == arg.end() || arg["window"] <= 0)     ? ( SegWindow = 12 ) : ( SegWindow = arg["window"] );
  (arg.find("hicut") == arg.end() ||  arg["hicut"] < 0)      ? ( SegHicut = 2.5 ) : ( SegHicut  = arg["hicut"] );
  (arg.find("locut") == arg.end() || arg["locut"] < 0)       ? ( SegLocut = 2.2 ) : ( SegLocut  = arg["locut"] );
  (arg.find("maxXes") == arg.end() || arg["maxXes"] < 0) ? ( MaxX = 0)   : ( MaxX  = arg["maxXes"]);
  (arg.find("maxtrim") == arg.end() || arg["maxtrim"] < 0)   ? ( MaxTrim = 100 )    : ( MaxTrim   = arg["maxtrim"]);
  (arg.find("period") == arg.end() || arg["period"] < 1)     ? ( Period = 1 )     : ( Period    = arg["period"]);
  (arg.find("merge") == arg.end() || arg["merge"] < 1)     ? ( MergeOverlaps = 1 )     : ( MergeOverlaps    = arg["merge"]);
